...
function setupContent() {
  ...
  if (deploy === false) {
    ...
    // destination_formatted = 'Palm Beach'
    ...
    if (showDefault) {
      ...
      if (myCityCharLength > 22) {
        myCity[0].style.fontSize = "7px"
        myCity[1].style.fontSize = "7px"
        to[0].insertAdjacentHTML('afterend', '<br>')
        h4[0].style.lineHeight = "9px"
...
function animate() {
  ...
  tl.set(["#main_content"], { autoAlpha: 0, force3D: true })
    .set(["#cta"], { force3D: true, rotation: .001 })
    .addLabel('frame_4')
    // .from(['#main_content'], .6, { y:"+=250", ease: Back.easeOut.config(.3)})
    .staggerTo(['#main_content, #h4', '#priceHolder', '#cta','#terms'], 0.5, { autoAlpha: 1, ease: Power1.easeInOut }, 0.3, 'frame_4+=0.5')
